...
{
    struct MINIFS_fs_super_block *disk_sb;
                        ...
                        struct buffer_head *bh, int create)
{
    ...
    printk(KERN_INFO "MINIFS: get block [%lu] of inode [%llu]\n", block,
           inode->i_ino);
    if (block > MINIFS_N_BLOCKS)
    {
        return -ENOSPC;
    }
...
int alloc_block_for_inode(struct super_block *sb, struct MINIFS_inode *p_H_inode, ssize_t size)
{
    ...
    if (size + p_H_inode->blocks > MINIFS_N_BLOCKS)
    {
        return -ENOSPC;
    }
    ...
    {
        uint64_t empty_blk_num = MINIFS_find_first_zero_bit(bmap, disk_sb->blocks_count / 8);
        p_H_inode->block[p_H_inode->blocks] = empty_blk_num;
        p_H_inode->blocks++;
        uint64_t bit_off = empty_blk_num % (MINIFS_BLOCKSIZE * 8);
